...
class SkyDataService:
    ...
    _OVO_API_FILE_LIST = {
        ...
        "jl": "/api/sky/jlwz/jl",     # "/api/sky/rwby/jl",
    ...
    def get_sky_sg(self, code):
        ...
        url = f"{self._OVO_API}/api/sky/sgwz/sgv1?key={self.ovo_key}&id={code}"  # 第一次好友码，后续长ID
        res = Http.send_request('GET', url)
        text = res
    ...
    def get_hs_txt(self):
        ...
        if '降落时间' in content_data:
         content += f"\r\n  {content_data}"
        text = (f"【Sky红石降落】- {res.get('time')}" + content) if content else ""
