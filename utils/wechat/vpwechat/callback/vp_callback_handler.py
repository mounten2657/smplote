...
class VpCallbackHandler(VpBaseFactory):
    ...
    def on_message(self, data):
        ...
        logger.debug(f"on message: {message}", 'VP_CALL_ON_MSG')
        if not message:
            logger.error(f"on message: message body is not json string!", 'VP_CALL_ON_MSG_ERR')
            return False
        ...
        if f_wxid:
            if msg_type == 51:  # 同步消息
                logger.warning(f"on message: 忽略消息[T1]<{msg_id}> 来自 <{f_wxid}>", 'VP_CALL_ON_MSG_SKP')
                return False
            if 'gh_' in f_wxid:  # 公众号
                logger.warning(f"on message: 忽略消息[T2]<{msg_id}> 来自 <{f_wxid}>", 'VP_CALL_ON_MSG_SKP')
                return False
            if f_wxid in str(app_config['g_wxid_exc']).split(','):  # 无用群
                logger.warning(f"on message: 忽略消息[T3]<{msg_id}> 来自 <{f_wxid}>", 'VP_CALL_ON_MSG_SKP')
                return False
            if f_wxid not in str(a_g_wxid).split(',') and not (is_my or is_sl):  # 仅限特定群、自己发的消息 和 私聊
                logger.warning(f"on message: 忽略消息[T4]<{msg_id}> 来自 <{f_wxid}>", 'VP_CALL_ON_MSG_SKP')
                return False
    ...
    def rev_handler(params):
        ...
        if g_wxid:
            ...
            msg['send_wxid_name'] = send_user.get('remark_name', send_user.get('nickname', 'null'))
            msg['to_wxid_name'] = to_user.get('remark_name', to_user.get('nickname', 'null'))
    ...
    def extract_at_user(msg_source):
        ...
        try:
            ...
            return at_user_node.text if at_user_node is not None else None
